...
def calculate_abundance(list_of_occurance):
	sum_all = 0
	abundance = []
	for occurance in list_of_occurance :
		sum_all += occurance
	for occurance in list_of_occurance :
		abundance.append(("%.5f" % (occurance/float(sum_all))))
	return abundance
#####################################################################
def gather_clone_clonotype_info(Dicoresult,dicoSeq,output_file):
	...
	for c in Dicoresult.keys():
		...
		clonal_info[c] = [("%.5f" % clone_abundance), dicoSeq[seq[0]][0],dicoSeq[seq[0]][1]]
		for s in seq :
			if dicoSeq[s][2] in clonotypes[c].keys() :
				clonotypes[c][dicoSeq[s][2]]. append(s)
			else:
				clonotypes[c][dicoSeq[s][2]] =[s]
				clonal_info[c].append(dicoSeq[s][2])
	for clone in clonotypes.keys() :
		list_clonotypes =[]
		for clonotype in clonotypes[clone].keys():
			list_clonotypes.append(len(clonotypes[clone][clonotype]))
		clonal_info[clone].append(calculate_abundance(list_clonotypes))
	return clonal_info,clonotypes
#####################################################################
def write_clonal_info(clonal_info_dico,output_file):
	...
	for clone in clonal_info_dico.keys():
		...
		for clonotype_abundance in  clonal_info_dico[clone][-1]:
			phrase += " " + str(clonotype_abundance)
	...
	for clone in clonotype_dico.keys():
		...
		for clonotype in clonotype_dico[clone].keys():
			for seq in clonotype_dico[clone][clonotype] :
				phrase = str(clone) + "_" + str(clonotype_number) + "\t" + str(seq) + "\t" + dicoSeq[seq][0] + "\t" + dicoSeq[seq][1] + "\t" + dicoSeq[seq][2] + "\t" + dicoSeq[seq][3] + "\n"   
				filetowrite.write(phrase)
			clonotype_number += 1
	filetowrite.close()
	return 0
