...
#write_fasta_from_dico(dico,output_file,type_of_fasta)
#read_fasta(filename)
#####################################################################
def read_file (nomFi):
	f=open(nomFi,"r")
...
def write_fasta_from_dico(dico,output_file,type_of_fasta):
	file_name = output_file.replace('_seq_Fo_V_CDR3_Jseq.txt','') + "_" + type_of_fasta + ".fa"
	filetowrite=open(file_name,"w")
	for key in dico.keys():
		name = ">"+key+"\n"
		filetowrite.write(name)
		nuc_seq = dico[key]+"\n"
		filetowrite.write(nuc_seq)
		#print (name)
	filetowrite.close()
	return 0
#####################################################################
...
def write_clone_V_cdr3_(Dicoresult,dicoSeq,uniq_seq_dico,output_file):
	...
	filetowrite.close()
	return 0
def write_clone_V_cdr3_no_singleton(Dicoresult,dicoSeq,uniq_seq_dico,output_file):
	file_name = output_file.split(".")[0]+"_clone_V_CDR3_J_no_singleton.txt"
	filetowrite=open(file_name,"w")
	clone_number = 0
	list_no_singleton =[]
	c=0
	for cluster in Dicoresult.keys():
		if len(Dicoresult[cluster]) ==1 and uniq_seq_dico[Dicoresult[cluster][0]] ==[]:
			#print(Dicoresult[cluster])
			c+=1
		else :
			for seq in Dicoresult[cluster]:
					list_no_singleton.append(seq)
					sequence = str(clone_number) + "\t" + str(seq) + "\t" +dicoSeq[seq][0] + "\t" + dicoSeq[seq][1] + "\t" +dicoSeq[seq][2].split(" ")[0] +"\t" +dicoSeq[seq][3]+"\t" +dicoSeq[seq][4]+ "\n"
					filetowrite.write(sequence)
					if len(uniq_seq_dico[seq]) != 0 :
						for dup in uniq_seq_dico[seq] :
							list_no_singleton.append(dup)
							sequence = str(clone_number) + "\t" + str(dup) + "\t" +dicoSeq[seq][0] + "\t" + dicoSeq[seq][1] + "\t" +dicoSeq[seq][2].split(" ")[0] +"\t" +dicoSeq[seq][3]+"\t" +dicoSeq[seq][4]+ "\n"
							filetowrite.write(sequence)
		clone_number += 1
	#print("number of singleton : ",c)
	filetowrite.close()
	return list_no_singleton

def compare_list_dico(liste,dico):
	common_dico = {}
	for key in dico.keys():
		if key in liste:
			common_dico[key] = dico[key]
	return common_dico


def write_fasta_nosingleton (list_no_singleton,output_file,uniq_fasta,all_fasta):
	uniq_fasta_no_singleton = compare_list_dico(list_no_singleton,uniq_fasta)
	all_fasta_no_singleton = compare_list_dico(list_no_singleton,all_fasta)
	write_fasta_from_dico(all_fasta_no_singleton,output_file,"all_no_singleton")
	write_fasta_from_dico(uniq_fasta_no_singleton,output_file,"uniq_no_singleton")
	return 0
